...
const UserManagementModal: React.FC<UserManagementModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleDeleteUser = (e: React.MouseEvent, id: string) => {
    ...
    if (window.confirm('해당 사용자를 삭제하시겠습니까?')) {
      const currentUsersString = localStorage.getItem('appUsers');
      if (!currentUsersString) return;

      const currentUsers: AppUser[] = JSON.parse(currentUsersString);
      const nextUsers = currentUsers.filter(user => user.id !== id);
      
      localStorage.setItem('appUsers', JSON.stringify(nextUsers));
      setUsers(nextUsers);
    }
  };

  const inputClass = "w-full bg-[#3f3f46] text-white border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-blue-500 transition-all outline-none placeholder-slate-500 font-bold";

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-[3rem] shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)] w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleAddUser} className="flex flex-wrap gap-6 items-end">
              ...
              <div className="flex items-center gap-3 pb-4 px-1">
                 <input
                  ...
                  className="w-6 h-6 bg-[#3f3f46] border-none rounded-lg focus:ring-blue-500 cursor-pointer accent-slate-600"
                />
                ...
                {users.map(user => (
                  <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-5 font-bold text-[#1e293b] flex items-center">
                      <div className="w-11 h-11 rounded-full bg-slate-50 flex items-center justify-center mr-4 text-slate-300 border border-slate-100 group-hover:bg-white transition-all">
                    ...
                    <td className="px-8 py-5 text-center">
                      {user.id !== 'admin' ? (
                        <button onClick={(e) => handleDeleteUser(e, user.id)} className="text-[#ef4444] hover:text-red-600 p-3 hover:bg-red-50 rounded-full transition-all inline-flex items-center justify-center active:scale-90">
